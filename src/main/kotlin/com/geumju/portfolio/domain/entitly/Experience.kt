...
class Experience(
    ...
    @JoinColumn(name = "experience_id") //매핑이 되는 컬럼이 뭔지 알려줌
    var details: MutableList<ExperienceDetail> = mutableListOf() // 1:N 관계, 빈 리스트를 만들어줌, 데이터를 넣고 뺴고가 가능

    // 저희가 필요로 하는 데이터를 한 번에 깔끔하게 서비스에서 가져올 수 있도록 이 엔티티 안에다가 묶어 줌
